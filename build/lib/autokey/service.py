...
from autokey.iomediator.io_mediator_wayland import IoMediatorWayland
...
class Service:
    ...
    def __init__(self, app):
        ...
        # Determine the display server and initialize accordingly
        self.use_wayland = False
        if os.environ.get('WAYLAND_DISPLAY'):
            try:
                import pywayland
                self.use_wayland = True
                logger.info("Wayland display detected. Using Wayland backend.")
            except ImportError:
                logger.warning("Wayland display detected but pywayland is not installed. Falling back to X11.")
        else:
            logger.info("Using X11 backend.")

    def start(self):
        if self.use_wayland:
            self.mediator = IoMediatorWayland(self)
        else:
